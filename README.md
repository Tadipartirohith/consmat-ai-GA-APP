...
The four apps arrived with a couple of things that stopped a clean self hosted build, and all of them are sorted out here. The ajv version that caused the "Cannot find module 'ajv/dist/compile/codegen'" crash on Create React App 5 is pinned to version 8. Each app talks to the API over a relative address, so it works behind the nginx forward with no hard coded backend URL. Each app's Docker image is a multi stage build: a Node stage compiles the React source and an nginx stage serves the finished static site, so the whole stack comes up from a fresh clone with a single `docker compose up -d --build` and there is no prebuilt output to keep in sync. The half finished backend folders that ship inside each frontend app are not used, because this repo's single backend already provides exactly what every app expects.
